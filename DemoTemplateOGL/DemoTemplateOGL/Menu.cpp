...
void Menu::InitGraph(Camera *camara){
    ...
    // Creamos la flecha
    Billboard2D* main = new Billboard2D((WCHAR*)L"textures/arrow.png", 6, 6, centerX, centerY - menuH * 0.5f + 25.0f, 0, camara);
    ...
    billBoard2D.push_back(
        new Billboard2D((WCHAR*)L"textures/Menu.png", 6, 6, centerX, centerY, 0, camara)
    );
...
int Menu::update(int& menuOption) {
    ...
    switch (menuOption) {
    case 1:
        translate = glm::vec3(arrowX, centerY + firstButtonOffsetY + (0 * rowSpacing), 0);
        break;
    case 2:
        translate = glm::vec3(arrowX, centerY + firstButtonOffsetY + (1 * rowSpacing), 0);
        break;
    case 3:
        translate = glm::vec3(arrowX, centerY + firstButtonOffsetY + (2 * rowSpacing), 0);
        break;
    case 4:
        translate = glm::vec3(arrowX, centerY + firstButtonOffsetY + (3 * rowSpacing), 0);
        break;
    default:
        menuOption = 1;
        translate = glm::vec3(arrowX, centerY + firstButtonOffsetY + (0 * rowSpacing), 0);
